...
val check : _ Sig.t -> Context.t -> CoreExpr.ce ->
  (Sort.sort, Error.t) result -> Effect.t -> typed_ce
...
val check_prog : Var.supply -> (SurfExpr.se, _, Var.t) Prog.t -> (typed_ce Sig.t * typed_ce Prog.core_prog, Error.located) result
(** Typecheck a complete program (elaborate + typecheck).
    Returns both the final core signature and the typed core program. *)

val check_decl : Var.supply -> _ Sig.t -> (SurfExpr.se, _, Var.t) Prog.decl -> (Var.supply * typed_ce Prog.core_decl, Error.located) result
...
val check_decl_multi : Var.supply -> _ Sig.t ->
  ...
  (Var.supply * typed_ce Prog.core_decl * Error.located list, Error.located) result
...
val check_spec_decl : Var.supply -> typed_ce Sig.t -> (SurfExpr.se, _, Var.t) Prog.decl -> (Var.supply * typed_ce Sig.t, Error.located) result
...
val extend_sig_with_header :
  typed_ce Sig.t ->
  (SurfExpr.se, _, Var.t) Prog.decl ->
  (typed_ce Sig.t, Error.located) result
