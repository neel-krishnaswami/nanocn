...
let is_check_sat (c : located_cmd) =
  match SmtSexp.shape c.cmd with
  | SmtSexp.List [hd] ->
    (match SmtSexp.as_atom hd with
     | Some (SmtAtom.Reserved SmtAtom.R_check_sat) -> true
     | _ -> false)
  | _ -> false
...
let rec walk config (ct : Constraint.typed_ct) parent next
  ...
  | Constraint.Atom e ->
    (* Wrap the negated obligation in [(push 1)]/[(pop 1)] so it
       doesn't pollute the assertion store for sibling obligations.
       Without the bracket, asserting [(not e)] would persist after
       [(check-sat)]; the first tautological obligation (e.g. an
       [own p == own p] reflexivity) makes the store inconsistent,
       and every subsequent [(check-sat)] is then vacuously [unsat].
       The pos-chain decls in [pre]/[post] also vanish on pop, but
       each obligation builds a fresh chain so that's fine. *)
    let* e' = SmtExpr.of_ce e in
    let n = next in
    let pre = scope_cmds n in
    let post = close_cmd n in
    ...
    let cs = located (list_of pos [res pos SmtAtom.R_check_sat]) in
    Ok ([push_cmd] @ pre @ [assert_cmd] @ post @ [cs] @ [pop_cmd], n + 1)
  | Constraint.Is (l, e) ->
    (* Constructor discriminators are auto-generated by Z3 as
       [is-<ctor-name>], and our [ctor_name] mangles to [D-L] (see
       [SmtSym.ctor_name] and [SmtPrelude.ctor_decl]). Mirror
       [SmtExpr.of_ce]'s [Inject]/[Case] cases by recovering [D] from
       the scrutinee's sort. The negated discriminator assertion is
       [(push)]/[(pop)]-bracketed for the same reason as [Atom]. *)
    (match Sort.shape (CoreExpr.sort_of_info (CoreExpr.info e)) with
     | Sort.App (dsort, _) ->
       let* e' = SmtExpr.of_ce e in
       let n = next in
       let pre = scope_cmds n in
       ...
       let push_cmd = located (list_of pos [res pos SmtAtom.R_push; num pos 1]) in
       let pop_cmd = located (list_of pos [res pos SmtAtom.R_pop; num pos 1]) in
       let tester = sym pos (Printf.sprintf "is-%s" (SmtSym.ctor_name dsort l)) in
     ...
     | _ ->
       Error (Format.asprintf
                "is(%a) constraint at %a: scrutinee sort must be a datatype application, got `%a`"
                Label.print l
                SourcePos.print pos
                Sort.print (CoreExpr.sort_of_info (CoreExpr.info e))))
...
let rec free_ce bound ce acc =
  ...
  | CoreExpr.Var v ->
    ...
    else VarMap.add v (CoreExpr.sort_of_info (CoreExpr.info ce)) acc
