...
(* [Util.Invariant_failure] (a compiler bug) is not caught here: it
   propagates to the top-level driver, which renders it as a bug
   report.  User-facing diagnostics flow through [Error.located] and
   never include internal-invariant payloads. *)
...
type decl_acc = {
  ...
  diags_rev : Error.located list;
}

let resolve_and_check_decl acc raw_decl =
  let ((resolved, _env), supply) =
    ElabM.run acc.supply (Resolve.resolve_decl [] raw_decl) in
...
let compile_file source ~file =
  ...
  let diags_rev =
    ...
    | Some (Ok prog) ->
      let (typed_e, _supply) = ElabM.run acc.supply (
        let open ElabM in
    ...
    | None ->
      empty_rfile_outcome ~decls:parsed_decls ?main_loc
        [Error.locate_opt ~loc:None
           (Error.parse_error ~msg:"missing `main` declaration")]
    | Some (Error e) ->
