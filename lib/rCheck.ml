...
let lookup_rf_m (rs : RSig.t) (f : string) : (CoreExpr.typed_ce, RProg.typed_rinfo, Var.t) RFunType.t option ElabM.t =
...
let mk_rinfo loc delta sort eff : RProg.typed_rinfo =
  (object
    method loc = loc
...
and synth_rpf (rs : RSig.t) (delta : RCtx.t) (rpf : RefinedExpr.parsed_rpf) : (checked_rpf * CoreExpr.typed_ce * CoreExpr.typed_ce * RCtx.t * Constraint.typed_ct) ElabM.t =
  ...
  | RefinedExpr.RMakeRet _ ->
    ElabM.fail (Error.cannot_synthesize ~loc:pos ~construct:"make-ret")
  | RefinedExpr.RMakeTake _ ->
    ElabM.fail (Error.cannot_synthesize ~loc:pos ~construct:"make-take")
  | RefinedExpr.RHole _ ->
    ElabM.fail (Error.cannot_synthesize ~loc:pos ~construct:"hole")
...
and synth_crt_impl (rs : RSig.t) (delta : RCtx.t) (eff : Effect.t) (crt : RefinedExpr.parsed_crt) : (checked_crt * (CoreExpr.typed_ce, RProg.typed_rinfo, Var.t) ProofSort.t * RCtx.t * Constraint.typed_ct) ElabM.t =
  ...
  | RefinedExpr.CCall (f, spine) ->
    ...
    (match rf_opt with
     ...
     | None ->
       ElabM.fail
         (Error.unknown_function ~loc:pos ~name:f))
        ...
        | _ ->
          ElabM.fail
            (Error.construct_sort_mismatch ~loc:binfo#loc
               ~construct:"open-take" ~expected_shape:"Pred _"
               ~got:pred_sort))
     | _ ->
       ElabM.fail
...
and check_crt_impl (rs : RSig.t) (delta : RCtx.t) (eff : Effect.t) (crt : RefinedExpr.parsed_crt) (pf : (CoreExpr.typed_ce, RProg.typed_rinfo, Var.t) ProofSort.t) : (checked_crt * RCtx.t * Constraint.typed_ct) ElabM.t =
  ...
  | RefinedExpr.CIf (_x, se, crt1, crt2) ->
    ...
    let rinfo = mk_rinfo pos delta (ProofSort.comp pf) eff in
    let checked = RefinedExpr.mk_crt rinfo (RefinedExpr.CIf (_x, ce, checked_crt1, checked_crt2)) in
...
and check_spine_inner rs delta eff spine domain codomain =
  ...
  let pos = binfo#loc in
  let spine_rinfo = mk_rinfo pos delta (ProofSort.comp codomain) eff in
...
and _check_tuple rs delta eff spine pf =
  let binfo = RefinedExpr.spine_info spine in
  let pos = binfo#loc in
  let tuple_rinfo = mk_rinfo pos delta (ProofSort.comp pf) eff in
    ...
    | ProofSort.DepRes { info = _; bound_var = y1; pred = ce1 } :: rest1,
      ...
      (match Sort.shape pred_sort with
       ...
       | _ ->
         ElabM.fail
           (Error.construct_sort_mismatch ~loc:pos
              ~construct:"dependent resource"
              ~expected_shape:"Pred _"
              ~got:pred_sort))
        ...
        (match RPat.rpat_shape rp with
         ...
         | RPat.RReturn lpat ->
           ...
           (match CoreExpr.shape pred' with
            | CoreExpr.Return ret_ce ->
              ...
              go expanded_elems expanded_pf delta
            | _ ->
              fail_pred_shape rp_loc "return pattern" "return ce" pred')
            ...
            | CoreExpr.Take ((x, _), ce1, ce2) ->
              ...
              (match Sort.shape ce1_sort with
               | Sort.Pred inner_sort ->
                 ...
                 go expanded_elems expanded_pf delta
               | _ ->
                 ElabM.fail (Error.structured ~loc:rp_loc
                   (Error.K_dep_res_not_pred { got = ce1_sort })))
            | _ ->
              fail_pred_shape rp_loc "take pattern" "take x = ce1; ce2" pred')
         ...
         | RPat.RFail lpat ->
           ...
           (match CoreExpr.shape pred' with
            ...
            | _ ->
              fail_pred_shape rp_loc "fail pattern" "fail" pred')
              ...
              (match Sort.shape ce1_sort with
               ...
               | _ ->
                 ElabM.fail (Error.structured ~loc:rp_loc
                   (Error.K_dep_res_not_pred { got = ce1_sort })))
            | _ ->
              fail_pred_shape rp_loc "let pattern" "let x = ce1; ce2" pred')
         ...
         | RPat.RIfTrue rpat ->
           ...
           (match CoreExpr.shape pred' with
            ...
            | _ ->
              fail_pred_shape rp_loc "iftrue pattern" "if ce then ce1 else ce2" pred')
         ...
         | RPat.RIfFalse rpat ->
           ...
           (match CoreExpr.shape pred' with
            ...
            | _ ->
              fail_pred_shape rp_loc "iffalse pattern" "if ce then ce1 else ce2" pred')
