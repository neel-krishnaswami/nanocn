(** Refined patterns (q): structured patterns for matching proof sorts.
    ...
    - [t]: an annotated sequence of qbase entries via the merged
      [q_shapeF] shape functor (analogous to [RefinedExpr.spineF])
...
(** Shape functor for the top-level pattern (sequence of qbase
    entries).  Each cons case carries the head element directly
    (analogous to [RefinedExpr.spineF]) and the recursive tail
    position ['q]. *)
...
val map_q_shapeF : ('q1 -> 'q2) ->
  ('cpat, 'lpat, 'rpat, 'q1) q_shapeF ->
  ('cpat, 'lpat, 'rpat, 'q2) q_shapeF
(** [map_q_shapeF f s] applies [f] at the recursive tail position only.
    Head elements (cpat / lpat / rpat) pass through unchanged. *)
...
val mk       : 'b -> (('b, 'var) cpat, ('b, 'var) lpat, ('b, 'var) rpat, ('b, 'var) t) q_shapeF -> ('b, 'var) t
...
val shape       : ('b, 'var) t     -> (('b, 'var) cpat, ('b, 'var) lpat, ('b, 'var) rpat, ('b, 'var) t) q_shapeF
