  ...
  Alcotest.run "nanocn" [
    ...
    ("parse", [
      ...
      Alcotest.test_case "parse arith primitive" `Quick (fun () ->
        match Parse.parse_expr "Add([x, y])" ~file:"test" with
        | Ok e ->
          (match Expr.shape e with
           | Expr.App (Prim.Add, _) -> ()
      ...
      Alcotest.test_case "parse state op with type" `Quick (fun () ->
        ...
        | Ok e ->
          (match Expr.shape e with
           | Expr.StateOp (Prim.New, ty, _) ->
             (match Typ.shape ty with
              | Typ.Int -> ()
              | _ -> Alcotest.fail "expected int type param")
           | _ -> Alcotest.fail "expected StateOp New")
        | Error msg -> Alcotest.fail msg);
      ...
      Alcotest.test_case "parse Get with ptr type" `Quick (fun () ->
        ...
        | Ok e ->
          (match Expr.shape e with
           | Expr.StateOp (Prim.Get, _, _) -> ()
           | _ -> Alcotest.fail "expected StateOp Get")
        | Error msg -> Alcotest.fail msg);
    ...
    ("typecheck", [
      Alcotest.test_case "synth int literal" `Quick (fun () ->
        ...
        | Error msg -> Alcotest.fail msg
        | Ok e ->
          match Typecheck.synth Context.empty e with
          | Ok te ->
      ...
      Alcotest.test_case "check tuple" `Quick (fun () ->
        ...
        | Ok e ->
          ...
          match Typecheck.check Context.empty e pair_ty Effect.Pure with
          | Ok te ->
      ...
      Alcotest.test_case "check let + Add" `Quick (fun () ->
        match Parse.parse_expr "let x = 1; Add([x, x])" ~file:"test" with
        | Error msg -> Alcotest.fail ("parse: " ^ msg)
        | Ok e ->
          let int_ty = Typ.In (Typ.Int, object method loc = SourcePos.dummy end) in
          match Typecheck.check Context.empty e int_ty Effect.Pure with
          | Ok te ->
            (match Typ.shape (typ_of te) with
             | Typ.Int -> ()
      ...
      Alcotest.test_case "Div is effectful" `Quick (fun () ->
        match Parse.parse_expr "Div([1, 2])" ~file:"test" with
        | Error msg -> Alcotest.fail ("parse: " ^ msg)
        | Ok e ->
          match Typecheck.synth Context.empty e with
          | Ok te ->
      ...
      Alcotest.test_case "let-tuple destructuring" `Quick (fun () ->
        match Parse.parse_expr "let [a, b] = ([1, 2] : [int, int] [pure]); Add([a, b])" ~file:"test" with
        | Error msg -> Alcotest.fail ("parse: " ^ msg)
        | Ok e ->
          let int_ty = Typ.In (Typ.Int, object method loc = SourcePos.dummy end) in
          match Typecheck.check Context.empty e int_ty Effect.Pure with
      ...
      Alcotest.test_case "Get with explicit type" `Quick (fun () ->
        ...
        | Ok e ->
          let int_ty = Typ.In (Typ.Int, object method loc = SourcePos.dummy end) in
          match Typecheck.check Context.empty e int_ty Effect.Effectful with
          | Ok _ -> ()
          | Error msg -> Alcotest.fail msg);
      ...
      Alcotest.test_case "Set with explicit type" `Quick (fun () ->
        ...
        | Ok e ->
          ...
          match Typecheck.check Context.empty e unit_ty Effect.Effectful with
          | Ok _ -> ()
          | Error msg -> Alcotest.fail msg);

      Alcotest.test_case "annotation effect mismatch fails" `Quick (fun () ->
        match Parse.parse_expr "Div([1, 2]) : int [pure]" ~file:"test" with
        | Error msg -> Alcotest.fail ("parse: " ^ msg)
        | Ok e ->
          match Typecheck.synth Context.empty e with
      ...
      Alcotest.test_case "typed output carries context" `Quick (fun () ->
        (* Parses as Annot(Let(x, 1, x), int, pure) *)
      ...
      Alcotest.test_case "typed output carries types on subterms" `Quick (fun () ->
        match Parse.parse_expr "Add([1, 2])" ~file:"test" with
        | Error msg -> Alcotest.fail ("parse: " ^ msg)
        | Ok e ->
          match Typecheck.synth Context.empty e with
          | Error msg -> Alcotest.fail msg
